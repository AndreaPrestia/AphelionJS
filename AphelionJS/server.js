...
http.createServer(async (req, res) => {
    await _process(req, res).catch(error => console.log(error));
}).listen(port);

const _getResource = (url, method) => {
...
const _process = async (req, res) => {
    const rethrowOnError = _getElement('rethrowOnError');
    ...
    try {
        const resource = _getResource(req.url, req.method);
        ...
        if (!resource.isPublic) {
            const tokenContent = _verifyToken(req);

            const claimPath = _getElement('claimPath');
...
const _verifyToken = (req) => {
    ...
    try {
        const decoded = jwt.verify(token, _getElement('serverKey'));
        return decoded;
    } catch (err) {
